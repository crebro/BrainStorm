from brainstorm.constants import COLORS
import pygame

"""
Message for tech with tim
Please don't ban me for this
This is the code of a library but I couldn't find a way to download it
so I copied and pasted it here myself
This code is taken from this link: https://github.com/DanBradbury/txtlib
...
class Text:
    def __init__(self, size, font, font_size=16, text=""):
        ...
        self.background_color = COLORS["black_background"]
        ...
        self.default_color = COLORS["white_foreground"]
        self.fonts = {}
                ...
                if code[id] not in ("<", ">") and not processing:
                    ...
                    if code[id + 1] != "/":
                        if code[id + 1].lower() == "b":
                            self.points.append(
                                Point(BEGIN, len(self.text) + txt_id, BOLD)
                            )
                        elif code[id + 1].lower() == "i":
                            self.points.append(
                                Point(BEGIN, len(self.text) + txt_id, ITALIC)
                            )
                        elif code[id + 1].lower() == "u":
                            self.points.append(
                                Point(BEGIN, len(self.text) + txt_id, UNDERLINE)
                            )
                        else:
                            ...
                            if code[id:end].find("color") != -1:
                                ...
                                self.points.append(
                                    Point(BEGIN, len(self.text) + txt_id, SIZE, size)
                                )
                            elif code[id:end].lower().find("font") != -1:
                                font = code[id + 7 : end - 1]
                                self.points.append(
                                    Point(BEGIN, len(self.text) + txt_id, FONT, font)
                                )
                    else:
                        if code[id + 2].lower() == "b":
                            self.points.append(
                                Point(END, len(self.text) + txt_id, BOLD)
                            )
                        elif code[id + 2].lower() == "i":
                            self.points.append(
                                Point(END, len(self.text) + txt_id, ITALIC)
                            )
                        elif code[id + 2].lower() == "u":
                            self.points.append(
                                Point(END, len(self.text) + txt_id, UNDERLINE)
                            )
                        elif code[id + 2].lower() == "c":
                            self.points.append(
                                Point(END, len(self.text) + txt_id, COLOR, None)
                            )
                        elif code[id + 2].lower() == "s":
                            self.points.append(
                                Point(END, len(self.text) + txt_id, SIZE, None)
                            )
                        elif code[id + 2].lower() == "f":
                            self.points.append(
                                Point(END, len(self.text) + txt_id, FONT, None)
                            )
    ...
    def load_font(self, font, size):
        if font in pygame.font.get_fonts():
            ...
            if self.fonts.__contains__(font):
    ...
    def check_fonts(self):
        ...
        for point in self.points:
            ...
            if not self.fonts.__contains__(font[len(font) - 1]):
                self.load_font(font[len(font) - 1], size[len(size) - 1])

            if not self.fonts[font[len(font) - 1]].__contains__(size[len(size) - 1]):
                self.load_font(font[len(font) - 1], size[len(size) - 1])
    ...
    def update(self):
        ...
        for line in self.text.split("\n"):
            ...
            for point in self.points:
                if char <= point.pos <= char + len(line):
                    ...
                    if -self.text_start <= dist_from_top <= bottom_border:
                        ...
                        try:
                            text = self.replace(
                                line[point.pos - char : self.points[id + 1].pos - char]
                            )
                        except:
                            text = self.replace(line[point.pos - char :])
            ...
            if not printed and -self.text_start <= dist_from_top <= bottom_border:
                ...
                self.area.blit(
                    render,
                    (x, dist_from_top + self.text_start + line_pos + self.border),
                )
