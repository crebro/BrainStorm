...
class NewsScreen:
    def __init__(self, surface) -> None:
        ...
        self.topText = FONTS["Bold"].render(
            "Latest Health News", 1, COLORS["white_foreground"]
        )
        self.topPadding = 20
        ...
        for news_item in self.news:
            ...
            try:
                ...
                image = pygame.transform.scale(
                    pygame.image.load(randomStorageLocation),
                    (self.newsImageWidth, self.newsImageHeight),
                )
            ...
            imageX, imageY = (
                self.width / 2
                - (self.allowedWidthForNews / 2)
                + currentRowIteration * self.newsWidth
                + (
                    self.newsPadding
                    if currentRowIteration == 1
                    else self.newsPadding - 2
                ),
                currentHeight,
            )
            textX, textY = (
                imageX + (image.get_width() / 2),
                currentHeight + image.get_height() + 10,
            )
