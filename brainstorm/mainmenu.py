...
class MainMenu:
    def __init__(self, surface) -> None:
        ...
        self.mainMenuByline = FONTS["Regular"].render(
            "The all in one solution to Health Problems and Goals",
            1,
        ...
        self.gamesOption = MenuButton(
            IMAGES["console"],
            "Brain Games",
            self.width / 2 - (IMAGES["console"].get_width()),
            self.height / 2 - (IMAGES["console"].get_height() / 2),
        )
        self.newsOption = MenuButton(
            IMAGES["news"],
            "Health News",
            self.width / 2 + (IMAGES["console"].get_width()),
            self.height / 2 - (IMAGES["console"].get_height() / 2),
    ...
    def update(self):
        ...
        self.surface.blit(
            self.mainMenuHeadline,
            (self.width / 2 - (self.mainMenuHeadline.get_width() / 2), self.topPadding),
        )
        self.surface.blit(
            self.mainMenuByline,
            (
                self.width / 2 - (self.mainMenuByline.get_width() / 2),
                self.mainMenuHeadline.get_height() + self.topPadding,
            ),
        )
